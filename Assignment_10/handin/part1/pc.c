...
uthread_mutex_t mx;
uthread_cond_t canAddOne;
uthread_cond_t canRemoveOne;
...
void* producer (void* v) {
    for (int i=0; i<NUM_ITERATIONS; i++) {
        ...
        uthread_cond_signal(canRemoveOne);
...
void* consumer (void* v) {
    for (int i=0; i<NUM_ITERATIONS; i++) {
        ...
        uthread_mutex_lock(mx);
        while (items == 0) {
            uthread_cond_wait(canRemoveOne);
        }
...
int main (int argc, char** argv) {
    ...
    canAddOne = uthread_cond_create(mx);
    canRemoveOne = uthread_cond_create(mx);
