...
uthread_sem_t mx;
uthread_sem_t canAddOne;
uthread_sem_t canRemoveOne;
...
void* producer (void* v) {
    for (int i=0; i<NUM_ITERATIONS; i++) {
        ...
        uthread_sem_signal(canRemoveOne);
    }
    return NULL;
}
...
int main (int argc, char** argv) {
    ...
    canAddOne = uthread_sem_create(0);
    canRemoveOne = uthread_sem_create(MAX_ITEMS);
